...
public interface PersonalMapper {
    ...
    @Insert("insert into tp_person_info(name, address, age, city, education, email, introduce, language, specialty, tel, cooperation_type, register_time) " +
            "values(#{name}, #{address}, #{age}, #{city}, #{education}, #{email}, #{introduce}, #{language}, #{specialty}, #{tel}, #{cooperationType}, #{registerTime})")
    @SelectKey(statement = "SELECT LAST_INSERT_ID()", keyProperty = "id", before = false, resultType = Integer.class)
